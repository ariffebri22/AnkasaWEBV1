...
function Search() {
    ...
    const filteredData = sortedFlightData.filter((flight) => {
        ...
        // Filter berdasarkan Transit
        if (!filtersTransit.direct && !filtersTransit.transit && !filtersTransit.transit2plus) {
            // Jika tidak ada filter Transit yang dipilih, tidak perlu memeriksa kondisi Transit
        } else {
            if (!((filtersTransit.direct && flight.transit === 0) || (filtersTransit.transit && flight.transit === 1) || (filtersTransit.transit2plus && flight.transit >= 2))) {
                // Setel isFlightMatched ke false jika kondisi Transit tidak terpenuhi
                isFlightMatched = false;
            }
        }

        // Filter berdasarkan Facilities (contoh: Baggage, Meal, Wifi)
        if (!filtersFacilities.baggage && !filtersFacilities.meal && !filtersFacilities.wifi) {
            // Jika tidak ada filter Facilities yang dipilih, tidak perlu memeriksa kondisi Facilities
        } else {
            ...
            if (!selectedFacilities.every((facility) => flight.facilities.includes(facility))) {
                // Setel isFlightMatched ke false jika kondisi Facilities tidak terpenuhi
                isFlightMatched = false;
            }
        }

        // Filter berdasarkan Departure Time (contoh: 00:00 - 06:00, 06:00 - 12:00, dst.)
        // Filter berdasarkan Departure Time (contoh: 00:00 - 06:00, 06:00 - 12:00, dst.)
        if (!filtersDepartureTime["0-6"] && !filtersDepartureTime["6-12"] && !filtersDepartureTime["12-18"] && !filtersDepartureTime["18-24"]) {
            // Jika tidak ada filter Departure Time yang dipilih, tidak perlu memeriksa kondisi Departure Time
        } else {
            ...
            if (
                ...
                // Penerbangan sesuai dengan salah satu kondisi Departure Time yang dipilih
            } else {
                // Setel isFlightMatched ke false jika kondisi Departure Time tidak terpenuhi
                isFlightMatched = false;
            }
        }

        // Filter berdasarkan Arrival Time (contoh: 00:00 - 06:00, 06:00 - 12:00, dst.)
        if (!filtersArrivalTime["0-6"] && !filtersArrivalTime["6-12"] && !filtersArrivalTime["12-18"] && !filtersArrivalTime["18-24"]) {
            // Jika tidak ada filter Arrival Time yang dipilih, tidak perlu memeriksa kondisi Arrival Time
        } else {
            ...
            if (
                ...
                // Penerbangan sesuai dengan salah satu kondisi Arrival Time yang dipilih
            } else {
                // Setel isFlightMatched ke false jika kondisi Arrival Time tidak terpenuhi
                isFlightMatched = false;
            }
        }

        // Filter berdasarkan Airlines (contoh: Garuda Indonesia, Air Asia, dst.)
        if (!filtersAirlines.garudaId && !filtersAirlines.airAsia && !filtersAirlines.lionAir && !filtersAirlines.singaporeAirlines && !filtersAirlines.Citylink) {
            // Jika tidak ada filter Airlines yang dipilih, tidak perlu memeriksa kondisi Airlines
        } else {
            ...
            if (!selectedAirlines.includes(flight.name)) {
                // Setel isFlightMatched ke false jika kondisi Airlines tidak terpenuhi
                isFlightMatched = false;
            }
        }

        return isFlightMatched; // Mengembalikan true jika setidaknya satu kondisi dari setiap filter terpenuhi
    });
                                ...
                                <div className={`overflow-hidden transition-max-height duration-500 ease-in-out ${openFacilities ? "max-h-[100px]" : "max-h-0"}`}>
                                    <div className="flex flex-col">
                                        <div className="flex flex-row justify-between w-full items-center mb-5">
                                            ...
                                            <input type="checkbox" name="" id="Baggage" className="w-4 h-4" />
                                        </div>
                                        <div className="flex flex-row justify-between w-full items-center mb-5">
                                            <label className="text-sm" htmlFor="meal">
                                                Meal
                                ...
                                <div className={`overflow-hidden transition-max-height duration-500 ease-in-out ${openAirlines ? "max-h-[300px]" : "max-h-0"}`}>
                                    <div className="flex flex-col">
                                        ...
                                        <div className="flex flex-row justify-between w-full items-center mb-5">
                                            ...
                                            <input type="checkbox" name="" id="singaporeAirlines" className="w-4 h-4" />
                                        </div>
                                        <div className="flex flex-row justify-between w-full items-center mb-5">
                                            <label className="text-sm" htmlFor="Citylink">
                                                Citylink
                                            </label>
                                            <input type="checkbox" name="" id="Citylink" className="w-4 h-4" />
                                        </div>
                                    </div>
                                </div>
                            </div>
